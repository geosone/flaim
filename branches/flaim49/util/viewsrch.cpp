...
//
// $Id: viewsrch.cpp 12334 2006-01-23 12:45:35 -0700 (Mon, 23 Jan 2006) dsanders $
//-------------------------------------------------------------------------
...
FLMINT ViewGetKey( void)
{
		  ...
		  (pLFile->uiLfType == LF_CONTAINER)))
	{
		...
		if ((!ViewGetNum( TempBuf, &Num, FALSE, 4,
				...
				(!ValEntered))
			return( FALSE);
		longToByte( Num, gv_ucViewSearchKey);
	...
	if (gv_uiViewSearchLfNum == FLM_DICT_INDEX)
	{
		...
		intToByte( (FLMUINT16)wTagType, &gv_ucViewSearchKey [1]);
