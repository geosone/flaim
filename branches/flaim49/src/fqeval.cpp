		...
		for (;;)
		{
			...
			if (uiRecFldNum == uiFldNum)
			{
				...
				if (bFound && pTreeAtom->val.QueryFld.fnGetField)
				{
					...
					else if (uiResult == FLM_UNK)
					{
						if (bHaveKey)
						{
							...
							// key in this case. If the callback does not know if
							// the field really exists, we must defer judgement
							// on this one until we can fetch the record. Hence,
							// we force the result to be UNKNOWN. Note that it
							// must be set to UNKNOWN, even if this is a field
							// exists predicate (!bGetAtomVals). If we set it to
							// NO_TYPE and fall through to exist, it would get
							// converted to a FLM_BOOL_VAL of FALSE, which is NOT
							// what we want.
							...
							// Better not be multiple results in this case
							// because we are evaluating a key.
				...
				if (bFound)
				{
					...
					{
						if ((pTmpResult->pNext = (FQATOM *) GedPoolCalloc( 
							pPool, sizeof( FQATOM))) == NULL)
						{
							rc = RC_SET( FERR_MEM);
							goto Exit;
						}
					...
					if ((rc = flmCurGetAtomVal( pRecord, pField, pPool, 
						eFldType, pTmpResult)) == FERR_CURSOR_SYNTAX)
					{
						goto Exit;
					}
				}
			...
			// value searched for was not found, unlink it from the result
			// list.
...
Exit:
	...
	// If no match was found anywhere, set the result to FLM_UNKNOWN if
	// field content was requested, or FLM_FALSE if field existence was to
	// be tested.
